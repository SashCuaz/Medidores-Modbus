/*
   The example will use packet1 to read a HOLDING REGISTER from Modbus address (command 0x04) for BADGER, ENDRESS, SIEMENS and EQUISYS meters,
   ...
   October, 20th, 2021
   Modified: June, 13th, 2022
   Ver0.4
   last tested: December 3rd 2021.
...
#define     SIEMENS_REG_FR     0x0BBB         // 4:03003 - LSB Tasa de Flujo [m3 / seg]    --Flow Rate instantaneus [m3/seg], float 32 bits
#define     SIEMENS_REG_TV     0x0BC6         // 4:03014 - LSB Volumen total + [m3]    -- Total volume + (accumulated) , double 64 bits
  ...
  switch(Modelo)
   {
    ...
    case ULTRATT:                // Modelo ULTRA TT  EQUYSIS       if modAfor == "ULTRATT"
     ...
     case SIEMENS:
      modAfor = "SITRANS FM MAG 6000";
      modbus_construct(packet1, 1, READ_HOLDING_REGISTERS, SIEMENS_REG_FR, 2, 1);  // ID=1, CMD=3 (Read HOLDING Reg), ModbusAdrr, Reg_read, Index_MasterReg)
      modbus_construct(packet2, 1, READ_HOLDING_REGISTERS, SIEMENS_REG_TV, 4, 3);  // Packet 2 point to next Modbus regiser, in this case Flow rate  
      break;
...
void datosModbus(MODEL Modelo){
  uint32_t tmp = 0x00;
  uint32_t trans = 0x00;
  float64_t trans2_64, tmp2_64, tmp64, trans64;
  char *str;
  delay(100);
  Serial.println(); 
  // get the new byte:
  ...
  //    Serial.println(str);
        tmp64 = regs[3];
        trans2_64 = trans2_64 + tmp2_64 + trans64 + tmp64 ;
  ...
  //      Serial.println(str);
    ...
    case ULTRATT:                // Modelo ULTRA TT  EQUYSIS       if modAfor == "ULTRATT"
        ...
        */
        ...
        trans = tmp << 16 |  regs[5];
        Serial.println("Registros ");
        Serial.print(" -- Reg[4] HIGH High Word: ");
        Serial.println(regs[4],HEX);
        Serial.print(" -- Reg [5] LOW High Word: ");
        Serial.println(regs[5],HEX);
        Serial.print(" -- Reg[6] Unit and fractional part: ");
        Serial.println(regs[6],HEX);
        uint8_t unidades = regs[6] >> 8;
        ...
        uint8_t fraction = regs[6] | 0x0F ;           // Like if it was and(|) 0x0F
        ...
        Serial.println(regs[2],HEX);
        Serial.print(" -- Reg[3] Unit and fractional Word: ");
        Serial.println(regs[3],HEX);
     ...
     case SIEMENS:                          // Big endian byte format
    ...
    // Read 4 Input Registers starting at 403003 (MODBUS ADDRESS)
    // Serial.print("  --Inst. measured fluid Flow Rate [m3/h] = ");
    // Serial.println(flowRate);
    // Serial.print("  --Inst. measured fluid Flow Rate [m3/s] float = ");
    // Serial.println(flowRate/3600,5);
